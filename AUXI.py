...
if page == "REPORTE":
    ...
    def _plotly(fig, key: str, uirev: str):
        ph = st.empty()
        if fig is None:
            with ph:
    ...
    def _df(df, key: str, caption: str | None = None):
        ph = st.empty()
        with ph:
            if caption:
                st.caption(caption)
    ...
    with hcol1:
        ...
        _plotly(fig_ent_h, key="fig_horiz_ent", uirev="horiz_ent")
    with hcol2:
        ...
        _plotly(fig_prog_h, key="fig_horiz_prog", uirev="horiz_prog")
    ...
    with ucol1:
        ...
        _plotly(fig_ent_univ, key="fig_univ_ent", uirev="univ_ent")
    with ucol2:
        ...
        _plotly(fig_prog_univ, key="fig_univ_prog", uirev="univ_prog")
    ...
    with c1:
        ...
        _plotly(fig_top_ent_l3, key="fig_top_l3_ent", uirev="top_l3_ent")
    with c2:
        ...
        _plotly(fig_top_prog_l3, key="fig_top_l3_prog", uirev="top_l3_prog")
    ...
    with c3:
        ...
        _plotly(fig_top_ent_l2, key="fig_top_l2_ent", uirev="top_l2_ent")
    with c4:
        st.markdown("#### Programa — Top 8 L2")
        fig_top_prog_l2 = plotly_topN_por_candidato_lvl2(res_prog_lvl2, cand_sel_l2, N=8, usar="share")
        _plotly(fig_top_prog_l2, key="fig_top_l2_prog", uirev="top_l2_prog")
    ...
    with colA:
        try:
            ...
            _plotly(fig_low_ent_l3, key="fig_low_l3_ent", uirev="low_l3_ent")
    ...
    with colB:
        try:
            ...
            _plotly(fig_low_prog_l3, key="fig_low_l3_prog", uirev="low_l3_prog")
        except Exception as e:
            st.warning(f"No fue posible calcular: {e}", icon="⚠️")
    ...
    with colC:
        try:
            ...
            _plotly(fig_low_ent_l2, key="fig_low_l2_ent", uirev="low_l2_ent")
        except Exception as e:
            st.warning(f"No fue posible calcular: {e}", icon="⚠️")
    with colD:
        try:
            ...
            _plotly(fig_low_prog_l2, key="fig_low_l2_prog", uirev="low_l2_prog")
        except Exception as e:
            st.warning(f"No fue posible calcular: {e}", icon="⚠️")
    ...
    _plotly(fig_exec, key="fig_exec_cov_div", uirev="exec_cov_div")
    ...
    # placeholders para mantener árbol estable
    ...
    try:
        ...
        if P_pilar is None or P_pilar.empty:
            ...
            with ph_coh_heat:
                _plotly(fig_heat, key="fig_coh_heat", uirev="coh_heat")
            ...
            with ph_coh_bar:
                _plotly(fig_global, key="fig_coh_bar", uirev="coh_bar")
                st.caption("Barra = coherencia promedio ENT↔PROG (0–1) ponderada por soporte común.")

            with ph_coh_tab:
                _df(agg_global, key="df_coh_global", caption=None)
    ...
    try:
        ...
        with ph_ts_plot:
            _plotly(fig_ts, key="fig_deriva_ts", uirev="deriva_ts")
        with exp_ts:
            _df(ts_ent_prog, key="df_deriva_ts")
    ...
    try:
        ...
        if not res_tables:
            ...
            if df_pil.empty:
                ...
                with ph_vec_top:
                    _df(top_por_source, key="df_vec_top", caption="Top vecino por candidato (probabilidad máxima)")
                    ...
                    with ph_vec_det:
                        if cand_source is not None:
                            df_det = (df_pil[df_pil["source"]==cand_source]
                                        ...
                                        .head(top_k)
                                        .reset_index(drop=True))
                            _df(df_det, key="df_vec_det", caption=f"Vecinos de `{cand_source}` en `{pil_sel}`")
                        else:
                ...
                with ph_vec_heat:
                    _plotly(fig_hm, key="fig_vec_heat", uirev="vec_heat")
    ...
    try:
        ...
        if agg_cp.empty:
            ...
            if tipo_vista == "Heatmap":
                ...
                with ph_sent_plot:
                    _plotly(fig_hm, key="fig_sent_heat", uirev="sent_heat")
            else:
                ...
                with ph_sent_plot:
                    _plotly(fig_bar, key="fig_sent_bar", uirev="sent_bar")
        ...
        if not cands_disp:
            ...
            if df_cand.empty:
                ...
                with colL:
                    ...
                    _plotly(fig_pos, key="fig_top_pos", uirev="top_pos")
                with colR:
                    ...
                    _plotly(fig_neg, key="fig_top_neg", uirev="top_neg")

                with ph_toptab:
                    cols_show = ["candidato", name_col, "pilar", "n_chunks", "tokens", "stars_mean", "score_mean", "SBI"]
                    _df(df_cand[cols_show].sort_values(M, ascending=False), key="df_top_sent")
    ...
    try:
        ...
        if piv.size == 0:
            ...
            with ph_idx_heat:
                _plotly(fig_hm, key="fig_idx_heat", uirev="idx_heat")
        ...
        with ph_idx_bar:
            _plotly(fig_bars, key="fig_idx_bar", uirev="idx_bar")

        with ph_idx_tab:
            show_cols = ["candidato","POP_INDEX","INST_INDEX","TEC_INDEX","VAGO_INDEX","n_chunks","w_total"]
            _df(Cand[show_cols].sort_values(val_col, ascending=False), key="df_idx_tab")
                ...
                if vista_wc == "Por pilar → candidatos":
                    ...
                    if not pil_sel:
                        ...
                        if not keys:
                            ...
                            plt.tight_layout()
                            with ph_wc:
                                st.pyplot(fig, use_container_width=True, key="wc_plot_pilar")
                            plt.close(fig)
                    ...
                    if not cand_sel:
                        ...
                        if not keys:
                            ...
                            plt.tight_layout()
                            with ph_wc:
                                st.pyplot(fig, use_container_width=True, key="wc_plot_cand")
                            plt.close(fig)
    except Exception as e:
